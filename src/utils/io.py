...
class UndefineError(Exception):pass
...
class SeatingTable:
    def __init__(
        ...
        priory_path=constants.PRIORY_FILE,
        amend_path=constants.AMEND_FILE
    ):
        self.table_path = table_path
        self.name_path = name_path
        self.rule_path = rule_path
        self.output_path = output_path
        self.priory_path = priory_path
        self.amend_path = amend_path
        self.prob = []
        self.table = []
        self.names = {"other": []}
        self.rules = ({}, {})
        ...
        self.priory = []
        self.amend = []
        self.group:dict[str,list] = {}
        self._gro:dict[str,list] = {}
        self.read_table()
        self.read_names()
        self.read_rules()
        self.read_prioritization()
        self.read_amendment()    

    def read_amendment(self):
        self.amend = [[True for _ in range(max(self.table_num["ColumnOfGroup"]))] for _ in range(self.table_num["GroupNum"])]
        for _ in range(self.table_num["GroupNum"]):
            for __ in range(self.table_num["ColumnOfGroup"][_]):
                self.amend[_][__]=False
        _tmp=0
        with open(self.amend_path,'r',encoding="UTF-8") as f:
            for line in f.read().splitlines():
                for _ in range(self.table_num["GroupNum"]):
                    self.amend[_][_tmp]=not bool(int(line[_]))
                _tmp+=1

    def read_prioritization(self):
        with open(self.priory_path,"r",encoding="UTF-8") as f:
            for line in f.read().splitlines():
                self.priory.append(line.split('\n')[0])
    ...
    def read_names(self) -> None:
        ...
        idx = "other"
        with open(self.name_path,"r",encoding="UTF-8") as f:
            for line in f.read().splitlines():
